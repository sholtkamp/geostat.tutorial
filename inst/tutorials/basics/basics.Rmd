...
Die R-Befehlszeile kann auch als Taschenrechner genutzt werden. Der Zuweisungs-Operator `<-` wird dabei genutzt um  Werte, Objekte oder Funktionen in Variablen zu speichern. <br>
...
R bietet eine Vielzahl vordefinierter mathematischer Funktionen. Da einige von diesen ausgefallene Namen benutzen findest du hier eine Liste der wichtigsten Operationen. <br><br>

#### Arithmetik
```{r}
head <- c("Zeichen", "Funktion")
sign <- c("+", "-", "*", "/", "^", "%%")
name <- c("Plus", "Minus", "Mal", "Geteilt", "Exponent", "Modulo")

table <- data.frame(sign, name)
colnames(table) <- head
...
head <- c("Zeichen", "Funktion")
sign <- c("<", ">", "<=", ">=", "==", "!=")
name <- c("Kleiner", "Größer", "Kleiner gleich", "Größer gleich", "Gleich", "Ungleich")

table2 <- data.frame(sign, name)
...
head <- c("Zeichen", "Funktion")
sign <- c("!", "&&", "||")
name <- c("Logisch NOT", "Logisch AND", "Logisch OR")

table3 <- data.frame(sign, name)
colnames(table3) <- head
