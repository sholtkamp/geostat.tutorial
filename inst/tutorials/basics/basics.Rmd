...
```{r 2d_1, exercise=TRUE}
c(1, 3, 7) * c(5, 8, 2) 
```
```{r 2d_2, exercise=TRUE}
c(1, 3, 7) %*% c(5, 8, 2)
```
...
```
```{r 2f, exercise=TRUE, exercise.setup = "2f_setup"}
# Hier können Befehle getestet werden. "vec" ist vordefiniert.
vec

```
```{r 2f_question, echo=FALSE}
...
dim(vec)
```
Randnotiz: Die Ausgabe <code>NULL</code> des Befehls <code>dim(vec)</code> ist nicht gleichzusetzen mit 0. Stattdessen bedeutet <code>NULL</code>, dass der Rückgabewert nicht definiert ist.
