...
## 2. Vektoren
Vektoren sind eine der wichtigsten Datenstrukturen in R. Mit ihnen können Folgen beliebig vieler Elemente des selben Datentyps in einem Objekt gespeichert werden. So können z.B. Zahlenreihen oder auch Folgen von Begriffen in Vektoren gespeichert werden.
<br>
...
<br>
Ein Vektor mit Buchstaben (oder Worten) ist ebenso möglich: `b <- c("hallo", "R")`. Die Anführungsstriche sind dabei nötig, um R klarzumachen, dass es sich um Zeichenketten (auch bekannt als <code>Strings</code>) und nicht etwa um Variablennamen handelt. So würde hallo (ohne \"\") als Verweis auf ein Objekt mit dem Namen \"hallo\" verstanden werden (siehe unten). 
...
Beide Funktionen werden im folgenden Beispiel verdeutlicht:
```{r 2f, exercise=TRUE}
...
Datenhaltung
Konventionen: sprechende namen, kamelcase, underscores. englsih
