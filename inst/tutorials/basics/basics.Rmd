...
Dieses Tutorial findet in einer geschützten Umgebung statt. Deshalb sind einige Funktionen, die im normalen Workflow wichtig sind hier automatisiert um den Einstieg zu erleichtern. 
<br><br>
...
<br>
Die beiden Rauten haben dabei keine tiefere Bedeutung und führen jeden Ausgabe in der Konsole an. Die Zahl in den eckigen Klammern hingegen vermittelt die Information des führenden Indexes. Ein Index ist eine Positionsangabe, wobei Index 1 in R immer für die vorderste Stelle steht. Ein Beispiel dazu könnte folgendermaßen aussehen:
...
Zum Rechnen bietet R eine Vielzahl vordefinierter, mathematischer Operationen und Funktionen. Die wichtigsten von diesen für den Einstieg sind die folgenden:
```{r info_1_3}
...
R dient in den meisten Anwendungsfällen zur staistischen Analyse von Daten. Diese Daten werden in der Programmierumgebung in sogenannten Datenstrukturen gespeichert. R unterstützt eine Vielzahl von vorgefertigten Datenstrukturen. Um mit diesen arbeiten zu können, ist es nötig sowohl Datentypen, als auch die Datentrukturen grundsätzlich zu verstehen.
<br>


### Datentypen
Die Datentypen, die in R genutzt werden und für diese Veranstaltung wichtig sind, sind die folgenden:
<ul>
<li>`numeric`:<br> Numerische Werte sind Zahlen. Dieser Typ umfasst `integer` (ganze Zahlen) und `double` (relle Zahlen).</li>
<li>`logical`:<br> Logische Werte, also wahr (`TRUE`,`t`) und falsch (`FALSE`,`f`). Diese Werte werden allgemein auch als `Booleans` bezeichnet.</li>
<li>`character`:<br> Character Werte sind Zeichen, insbesondere auch Wörter oder Sätze.</li>
<li>`NA`:<br> NA Werte sind nicht vorhandene Werte. Diese stammen oft aus Fehlern in den Daten oder falschen Berechnungen.</li>
</ul>

Um den Typen einer Variablen oder eines Wertes heruaszufinden wird die Funktion `typeof(x)` benutzt. Die Erzeugung dieser Datentypen setzt auf bestimmte Konventionen. Diese sehen wie folgt aus:
```{r info_2.0, include = TRUE, echo = TRUE}
# Numeric
ex_int <- 42
ex_double <- 1.99 ## Double wird mit . erzeugt statt einem ,

# Logical
ex_bool <- TRUE ## Logische Werte werden komplett in Großbuchstaben geschrieben
ex_bool_2 <- FALSE
...
### Datenstrukturen
Mit diesen Datentypen als Grundlage können nun die Datenstrukturen betrachetet werden. Diese Grundlage ist notwendig, da die Datenstrukturen sich in zwei Gruppen einteilen lassen, basierend darauf, mit wie vielen Datentypen gleichzeitig sie kompatibel sind. Betrachte dazu die folgende Abbildung.
<br><br>
...
Wie hier zu sehen ist, können Vektoren, Matritzen und Arrays nur mit einem Datentyp gleichzeitig arbeiten. Dies führt dazu, dass R Typkonvertierungen durchführt, wenn man mehr als einen Datentypen zur Erzeugung nutzt. Erzeugt man beispielweise eine Vektor aus Zahlen (integer/double) und Wörtern (character) werden die Zahlen in Wörter umgewandelt. Das ist eine häufige Fehlerquelle wenn man beginnt mit R zu arbeiten.
<br>
In den folgenden Kapiteln werden diese Strukturen erklärt und Funktionen für den Umgang mit ihnen erläutert. In dieser Vorlesung sind davon insbesondere Vektoren und Dataframes von Bedeutung. 


## \ \ \ \ 2.1 Vektoren
Vektoren sind eine 1-dimensionale Datenstruktur, d.h. sie können genutzt werden, um Zahlenreihen oder auch Folgen von Begriffen zu speichern. Dabei können beliebig viele Elemente des selben Datentyps in einem Objekt gespeichert werden. Um einen Vektor zu erzeugen wird die Funktion `c()` [d.h. combine, kombiniere] benutzt. So kann zum Beispiel der Vektor `(1, 2, 3)` (hier also eine Zahlenreihe mit den Zahlen 1, 2 und 3) mit Hilfe von `c(1, 2, 3)` erzeugt werden. Dieser kann dann mit `a <- c(1, 2, 3)` der Variablen `a` zugewiesen werden.

Beispiele für Vektoren können wie folgt aussehen:
```{r info_2_1, echo = TRUE}
# Erzeuge das Temperaturbeispiel als Vektor
temp <- c(25, 24.5, 25, 25.5, 26)
...
# Erzeuge das Personenbeispiel als Vektor
versuchsperson_1 <- c("Mia", "Musterfrau", 25, 1.75, 5)
...
<br>
Die Beschreibung eines Mensche durch <code>versuchsperson_1 <- c("Mia", "Musterfrau", 25, 1.75, 5)</code> ist auch ein Vektor. Hier sind die Datentypen jedoch gemischt. `"Mia"` und `"Musterfrau"` sind vom Typen `character`, `25` und `5` sind `integer` und `1.75` ist ein `double`. Da jedoch verschiedene Datentypen in einem Vektor nicht gemischt werden können, führt R eine Typkonvertierung durch. Das heißt, dass in diesem Beispiel alle Werte in `character` umgewandelt werden.
<br>
Bei der Verwendung von Wörtern sind Anführungsstriche sind nötig, um R klarzumachen, dass es sich um Zeichenketten und nicht etwa um Variablennamen handelt. So würde Konsole (ohne \"\") als Verweis auf ein Objekt mit dem Namen <code>Konsole</code> verstanden werden (siehe unten). 
```{r info_2_1_2, exercise=TRUE}
b <- c("hallo", Konsole)
b
```
...
Im Umgang mit R tauchen Vektoren immer wieder auf. Auch komplexere Objekte können dabei in Vektoren zusammengefasst werden. Besonders häufig benötigt werden Vektoren mit speziellen Zahlensequenzen. Diese lassen sich in R einfach erzeugen. Hierzu stehen z.B. die Funktionen `seq()` und `rep()` zur Verfügung. 

Die Funktion `seq()` dient dazu eine regelmäßige Zahlenfolge zu erzeugen. Drei der folgenden Parameter müssen angegeben werden und der 4. Wert, normalerweise `length.out`, kann dann automatisch berechnet werden: <br>
<ul>
...
Um die Daten, die in einem Vektor gespeichert wurden wieder zugänglich zu machen, benötigt man Selektionsfunktionen. Dabei handelt es sich beispielsweise um die folgenden Funktionen. `Index_1` und `Index_2` sind dabei integers, `x` und `y` beliebige Zahlen.
<ul>
...
Vektoren verfügen über eine Vielzahl von Funktionen zur Manipulation. Im Folgenden werden die häufigsten dieser Funktionen aufgeführt. <code>x</code> bezeichnet dabei immer einen Vektor.
<ul>
...
## \ \ \ \ 2.2 Matrizen
Matrizen können als erweiterte Vektoren mit besonderen Eigenschaften angesehen werden und unterscheiden sich von diesen durch ihr Dimensions-Attribut `dim`. Während Vektoren dimensionslos sind, sind Matrizen grundsätzlich zweidimensional. 
<br>
...
Das Erzeugen und Selektieren bei Arrays funktioniert ähnlich wie bei Matrizen. Es werden lediglich mehr Argumente benötigt. Die Anzahl dieser zusätzlichen Argumente hängt von der Anzahl der Dimensionen ab.
...
## \ \ \ \ 2.5 Dataframes
Dataframes sind die häufigste Methode um in R größere Mengen an Daten systematisch zu speichern. Dataframes speichern i.d.R. Datentabellen Dabei stehen in den Zeilen die Beobachtungen und in den Spalten die Variablen, die für die Beobachtungen gemessen wurden. Sie sind zweidimensionale Strukturen, die sich intern aus einer Menge gleichlanger Vektoren zusammen setzen. Eine Beispiel für eine dem Dataframe analoge Struktur wäre eine Excel-Tabelle.
<br>
...
<br>
Um nachträglich Einträge zu einem Dataframe hinzuzufügen, können unter anderem `rbind()` [d.h. row bind, Zeile hinzufügen] und `cbind()` [d.h. column bin, Spalte hinzufügen] benutzt werden. 
<br>
...
numbers_df_extended
```
Bei dem Hinzufügen von Attributen ist wichtig, dass das neue Attribut für jede Beobachtung im Dataframe einen Wert hat. Deshalb ist es nicht möglich dem Dataframe einen `vec_english` mit weniger als 5 oder mehr als 5 Einträgen hinzuzufügen.
<br>
Darüber hinaus ist in dem Beispiel zu sehen, wie ein Dataframe von Natur aus mit `characters` umgeht: Sie werden in `factor`, also in Faktoren umgewandelt. Faktoren können dabei als Kategorien betrachtet werden. Versucht man einen `character` hinzuzufügen, der keiner der bereits vorhandenen Kategorien angehört, erzeugt man fehlende Werte (siehe nächstes Beispiel).
<br><br>
...
<br>
Bevor diese thematisiert werden, ist es jedoch sinnvoll zunächst Skalenniveaus zu thematisieren, da die Anwendbarkeit dieser Befehle von den Zieldaten abhängig ist.
...
<li>Mögliche Operationen: =, ≠, <, <=, >, >=, +, -, *, /</li>
<li>Beispiele: Grad Kelvin</li>
</ul>
</li>

### 3.2 Lagemaße
Lagemaße oder auch Maße der zentralen Tendenz geben Auskunft über den “üblichsten/häufigsten” Wert. Zu den Lagemaßen gehören:
<ul>
...
<li>`Interquartilsabstand`: Differenz zwischen dem dritten Quartil (0.75-Quantil) und dem ersten Quartil (0.25-Quantil); nicht anfällig gegen Ausreißer</li>
...
# Spannweite
range_vec <- range(vec)[2] - range(vec)[1]
range_vec
...
```
```{r 4_1x, exercise=TRUE, exercise.setup = "4_setup"}
...
```{r 4_2x, exercise=TRUE, exercise.setup = "4_setup"}
# Varaible mit Blütenblattlängen: petal_length
...
```{r 4_3x, exercise=TRUE, exercise.setup = "4_setup"}
# Varaible mit Blütenblattlängen: petal_length
