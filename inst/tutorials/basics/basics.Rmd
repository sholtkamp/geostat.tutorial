...
### Übung 1
Führe folgende Rechnungen durch und weise sie jeweils den Variablen zu. Wie lässt sich die letzte Rechnung durch Variablen einfacher in R ausführen?
<br>
...
<br>
Um einen Vektor zu erzeugen wird die Funktion `c()` benutzt. So kann zum Beispiel der Vektor `(1, 2, 3)` (hier also eine Zahlenreihe mit den Zahlen 1, 2 und 3) mit Hilfe von `a <- c(1, 2, 3)` erzeugt und der Variable `a` zugewiesen werden.<br>
Ein Vektor mit Buchstaben (oder Worten) ist ebenso möglich: `b <- c("hallo", "R")`. Die Anführungsstriche sind dabei nötig, um R klarzumachen, dass es sich um Zeichenketten und nicht etwa um Variablennamen handelt. So würde hallo (ohne "") als Verweis auf ein Objekt mit dem Namen "hallo" verstanden werden. Probiert mal `b <- c(hallo,R)`). Auch komplexere R-Objekte können in Vektoren zusammengefasst werden, es müssen nur alle Elemente in einem Vektor den selben Datentyp haben. 
<br>
